...
def login():
    if request.form["user_id"] != "" and request.form["user_pass"] != "" :
        encrypt_pass= encrypt_sha256(request.form["user_pass"])
        print(encrypt_pass)
        
        conn = pymysql.connect(host=host, user=user, password=password, db=db, cursorclass=pymysql.cursors.DictCursor)
        cursor = conn.cursor()
        
        sql="SELECT user_id, user_pass FROM user_profile where user_id=%s and user_pass=%s"
        try:
            print(encrypt_pass)
            print(request.form["user_id"])
            cursor.execute(sql,(request.form["user_id"],encrypt_pass))
            print(cursor.rowcount)
        except Exception as e:
            print (e)
        if(cursor.rowcount>0):
            res={"response":{"response": "Successfully Logged in"}}
...
def signup():
    ...
    cursor = conn.cursor()
    cursor2 = conn.cursor()
    if request.form['user_first_name'] == "" or request.form['user_last_name'] == "" or request.form['user_email'] == "" or request.form['user_id'] == "" or request.form['user_phone'] == "" or request.form['user_pass'] == "":
        res={"response": "Required field is empty!!!"}
        return jsonify(res)
    else:
        try:
            sql1 = "SELECT user_id FROM user_profile WHERE user_id = %s"
            cursor.execute(sql1,request.form["user_id"])
        except:
        ...
        for userall in user_data2:
            if userall["user_id"] == request.form["user_id"]:
                res={"response": "Duplicate User ID"}
                return jsonify(res)

        else:
            sql = "INSERT INTO user_profile(user_id,user_pass,user_first_name,user_last_name,user_email,user_phone) VALUES(%s, %s, %s, %s, %s,%s)"
            encrypt_pass= encrypt_sha256(request.form["user_pass"])
            
            try:

                cursor2.execute(sql,(request.form["user_id"],encrypt_pass,request.form["user_first_name"],request.form["user_last_name"],request.form["user_email"],request.form["user_phone"]))
                conn.commit()
...
def apartment_list():
    ...
    if request.form['location'] != "":
        sql="SELECT `apartment_name`, `apartment_address` as address, `apartment_contact` as contact, `apartment_image` as image_url, `apartment_avg_price` as avg_fare, `apartment_rating` as rating, `apartment_no_of_room` as no_of_room, `apartment_description` as description, apartment_uniq_id as apartment_id FROM `apartment_table` WHERE `apartment_location` = %s"
        try:
...
def confirmbooking():
    ...
    if request.form['user_id'] == "" or request.form['bed_id'] == "" or request.form['from_date'] == "" or request.form['to_date'] == "":
        res={"response": "Required field is empty!!!"}
        return jsonify(res)
    else:
        bed         = request.form['bed_id']
        apartment   = bed[0:3]
        room        = bed[0:6]
        payment     = 1
        # bed_status  = 1
        ...
        approval_status = paymentgateway(request.form['user_id'])
        print("I am")
        ...
        if approval_status == 1:
            try:
                ...
                booking_ref_no= ref_gen()
                payment_con_if = ref_gen()
                sql = "INSERT INTO booking_table (user_id, apartment_uniq_id, room_uniq_id, bed_uniq_id, booking_checkin_time, booking_checkout_time, booking_created_by, booking_confirmed_by, payment_status, bed_status, booking_ref_no, payment_confirm_id) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                cursor.execute(sql,(request.form['user_id'], apartment, room, bed, request.form['from_date'], request.form['to_date'], request.form['user_id'], request.form['user_id'], payment, 1, booking_ref_no, payment_con_if ))
                conn.commit()
                cursor.close()
                
                sql = "UPDATE bed_info SET bed_status=%s WHERE bed_uniq_id= %s"
                cursor2.execute(sql,(1, bed))
                conn.commit()
                cursor2.close()
                conn.close()
                ref= {"booking_ref_no": booking_ref_no}
